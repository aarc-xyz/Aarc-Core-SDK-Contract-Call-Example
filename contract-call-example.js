...
async function getMultichainBalance(address) {
    try {
        ...
        if (res.data) {
            //@ts-ignore
            const chainBalances = res.data.balances;
...
async function getDepositAddress({ contractPayload, fromToken, fromTokenAmount, }) {
    try {
        ...
        const baseToAmount = BigInt(Math.floor(+(requestedAmount ?? 0) * 10 ** (destinationToken.decimals ?? 18))).toString();
        const payload = {
            userOpHash: "",
            transferType,
        ...
        if (transferType === TransferType.WALLET) {
            ...
            const baseFromAmount = Math.floor(+(fromTokenAmount ?? 0) * 10 ** (fromToken.decimals ?? 18));
        ...
        else if (transferType === TransferType.ONRAMP ||
...
function generateCheckoutCallData(token, toAddress, amount) {
...
async function executeTransaction(depositData) {
    try {
        const tx = {
...
async function main() {
    try {
        ...
        const fromToken = balance[destinationToken.chainId.toString()]?.balances?.find((b) => b.symbol === "USDC") ?? null;
        console.log("Selected token:", fromToken);
        if (!fromToken) {
            throw new Error("No balance available for the token");
        }
        console.log("Generating call data...");
        const callData = generateCheckoutCallData(destinationToken.address, destinationWalletAddress, "10000");
